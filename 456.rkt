...
(define (place-queens a-board n)
  (local (; constant for no open spots on the board
          (define no-more-spots? (empty? (find-open-spots a-board)))
          ; constant for no more queens
          (define no-more-queens? (< n 1)))
    ;-IN-
    (cond
      [no-more-queens? (board-to-loqp a-board)] ; victory!
      [(and no-more-spots? (not no-more-queens?)) #f] ; reached a dead end
      [else 
       (local (; define a list of all possible results of choices
               (define list-of-maybe-loqp (map (lambda (qp) (place-queens (add-queen a-board qp) (- n 1))) (find-open-spots a-board)))
               ; [List-of [Maybe X]] -> X
               ; pulls the first item from a list that isn't false
               (define 
         ;-IN-
         (cond
           [ist-of-maybe-loqp
    ...
    [(and (empty? (find-open-spots a-board)) (< n 1)) (board-to-loqp a-board)]
    [(and (cons? (find-open-spots a-board)
    [(< n 1) a-board
    [(boolean? (place-queens (add-queen a-board (first (find-open-spots a-board))) (- n 1)) (place-queens (add-queen a-board (first (find-open-spots a-board))) (- n 1)) 

  
  (cond
    [(< n 1) a-board]
    [else (cons (place-queens (add-queen a-board (first qp)) (- n 1))


                                       
    [(and (cons? (find-open-spots a-board)) (< n 1)) ...] ;return the answer
  ...
  (local (; Board N -> [List-of [List-of QP]]
          ; takes a board and a number n of queens to add, and returns a list of all possible arrangements
  (cond
    [(< n 1) a-board]
    [else
     ...
     (cond
            (local (; [List-of QP] -> [Maybe [List-of QP]
                    ; define open spots as a list to run down
                    (define open-spots (find-open-spots a-board)))
              ;-IN-
              [(empty? open-spots) ...]
              [else (place-queens (add-queen
            [
          (place-queens (add-queen a-board (find-open-spots a-board))) (- n 1))]))
