...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |347|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
; a list ofall the one-argument function definitions in my definitions area
...
(check-error (lookup-def da-fgh 'square) "undefined function")
...
(check-error (eval-function* da-fgh (make-fun 'h (make-fun 'square 2))) "undefined function")
...
; S-expr -> BSL-fun-def
; parses a SL as a BSL-fun-def* assuming the former is a list of quoted BSL definitions.

(check-expect (da-parse '((define (f x) (+ 3 x))
                          (define (g y) (f (* 2 y)))
                          (define (h v) (+ (f v) (g v))))) da-fgh)
...
(check-error (def-parse '(define x (+ 3 x))) WRONG)
