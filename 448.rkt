...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |448|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
...
(define (find-path origination destination G)
  (local (; [List-of Node] Node Graph -> [Maybe Path]
          ...
          (define (find-path/list lo-Os)
            (cond
              [(andmap (lambda (origination) (false? (find-path origination destination G))) lo-Os) #false]
              [else (first (filter (lambda (origination) (not (false? (find-path origination destination G)))) lo-Os))]))
