...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname |Ex 115-sigs|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ())))
(require 2htdp/image)
...
; tank-x? predicate for TankPosition

(check-expect (tank-x? TANK-MIDDLE) true)
(check-expect (tank-x? TANK-LEFT) true)
(check-expect (tank-x? TANK-RIGHT) true)
(check-expect (tank-x? TANK-TOO-WIDE) false)
(check-expect (tank-x? TANK-NEG) false)
(check-expect (tank-x? NOT-TANK) false)
...
(check-expect (tank-check? tank-l-l) true)
(check-expect (tank-check? tank-l-r) true)
(check-expect (tank-check? tank-r-r) true)
(check-expect (tank-check? tank-m-r) true)
(check-expect (tank-check? tank-m-l) true)
(check-expect (tank-check? tank-m-0) true)

(check-expect (tank-check? tank-not) false)
(check-expect (tank-check? tank-wide) false)
(check-expect (tank-check? tank-neg) false)
...
(define (ufo? u)
  (posn? u)) ; I could figure out ranges but this overcomplicates things for now
...
(check-expect (sigs-check? aimstart) #true)
(check-expect (sigs-check? aimufodown1) #true)
(check-expect (sigs-check? aimok) #true)
(check-expect (sigs-check? aimlose) #true)
(check-expect (sigs-check? aimhalfway) #true)
(check-expect (sigs-check? firedstart) #true)
(check-expect (sigs-check? firedupone) #true)
(check-expect (sigs-check? firedmiss) #true)
(check-expect (sigs-check? fireddirecthit) #true)
(check-expect (sigs-check? firedclosehit) #true)
(check-expect (sigs-check? firedclosemiss) #true)
(check-expect (sigs-check? firedend) #true)

(check-expect (sigs-check? 5) #false)
; i could include more here but the expression should be really simple now.

(define (sigs-check? s)
  (cond
    [(sigs? s)(cond
                [(and (tank-check? (sigs-tank s)) (ufo? (sigs-ufo s)) (missile-or-not? (sigs-missile s))) #true]
                [else #false])]
    [else #false]))
